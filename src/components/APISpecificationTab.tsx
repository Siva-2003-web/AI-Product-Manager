...
export default function APISpecificationTab({
  apiSpec,
}: APISpecificationTabProps) {
                ...
                >
                  <div className="space-y-1 overflow-hidden">
                    <div className="flex items-center gap-2">
                      <span
                        className={`text-[9px] font-black px-1.5 py-0.5 rounded border ${getMethodBadge(ep.method)}`}
                      >
                        {ep.method}
                      </span>
                      <span
        ...
        <div className="md:col-span-7 bg-white border-2 border-slate-900 rounded-3xl p-6 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] flex flex-col justify-between min-h-[460px]">
          <div>
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
              <div className="flex items-center gap-2">
                <span
                  className={`text-xs font-black px-2 py-0.5 rounded border ${getMethodBadge(currentEndpoint.method)}`}
                >
            ...
            {/* Authentication note */}
            <div
              className={`p-3.5 rounded-xl border flex gap-3 text-xs mb-3.5 font-sans ${
                ...
                <>
                  ...
                  <div>
                    ...
                    <span className="block text-[11px] text-amber-600 mt-0.5">
                      Standard Bearer OAuth JWT token needs to be parsed in the
                ...
                <>
                  ...
                  <div>
                    ...
                    <span className="block text-[11px] text-emerald-600/80 mt-0.5">
                      Does not require access limits or verification headers to
                      query indices.
                    </span>
                  </div>
                </>
                    ...
                    <table className="w-full text-left text-xs text-slate-700 divide-y divide-slate-100">
                      ...
                      {currentEndpoint.queryParams.map((param, idx) => (
                        <tr key={idx} className="border-b border-slate-50">
                          ...
                          <td className="p-2">
                            <span
                              className={`font-bold ${param.required ? "text-rose-600" : "text-slate-400"}`}
                            >
